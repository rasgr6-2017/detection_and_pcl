...
class blobDetect:
  ...
  def callback(self,data):
    ...
    purpleBound = [([150, 0, 0], [170, 255, 255])]
	

	# # Red detection # #

    for (lower, upper) in redBound:
    ...
    for (lower, upper) in orangeBound:
        lower = np.array(lower, dtype = "uint8")
        upper = np.array(upper, dtype = "uint8")
    ...
    for (lower, upper) in purpleBound:
        lower = np.array(lower, dtype = "uint8")
        upper = np.array(upper, dtype = "uint8")
    ...
    # # Publish to the espeak node saying that it sees an object # #
