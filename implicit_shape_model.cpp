...
main (int argc, char** argv)
{
  ...
  // Loop to put all clouds in training vector
  for (unsigned int i_cloud = 0; i_cloud < number_of_training_clouds - 1; i_cloud++)
  {
    ...
    training_clouds.push_back (tr_cloud); // all training clouds
    training_normals.push_back (tr_normals); // normals of tr_clouds
    training_classes.push_back (tr_class); // class index
  }

  //Fast Point Feature Histogram descriptors used for feature estimation
  ...
  // template class
  // - size of the features (histograms) to compute
  // - type of points to work with
  // - type of normals to use
  pcl::ism::ImplicitShapeModelEstimation<153, pcl::PointXYZ, pcl::Normal> ism;

  // provide ism with training data
